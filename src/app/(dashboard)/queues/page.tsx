...
interface WorkerSupervisorStatus {
  version: number;
  host?: string;
  updatedAt: string;
  stale?: boolean;
...
export default function QueuesPage() {
  ...
  const [workerSupervisor, setWorkerSupervisor] = useState<WorkerSupervisorStatus | null>(null);
  ...
  useEffect(() => {
    if (sseData?.type === 'update' && sseData.queues) {
      ...
      if (sseData.workerSupervisor !== undefined) {
        setWorkerSupervisor(sseData.workerSupervisor);
      }
      setLoading(false);
    }
  }, [sseData]);
  ...
  useEffect(() => {
    ...
    const fetchSupervisor = async () => {
      try {
        ...
        setWorkerSupervisor(data.status || null);
      } catch (error) {
        console.error('Failed to fetch worker supervisor status:', error);
      }
    };
    fetchSupervisor();
    const interval = setInterval(fetchSupervisor, 30000);
    return () => clearInterval(interval);
  }, [isConnected]);
  ...
  const workerSupervisorSummary = workerSupervisor?.summary || { total: 0, ok: 0, warning: 0, down: 0 };
  const workerSupervisorIssues = workerSupervisor?.items?.filter((item) => item.status !== 'ok') || [];
  const workerSupervisorAlert = workerSupervisor?.stale || workerSupervisorIssues.length > 0;
  ...
  return (
    <div className="space-y-6">
      ...
      {workerSupervisorAlert && (
        <Card className={workerSupervisor?.stale ? 'border-yellow-500/50 bg-yellow-500/10' : 'border-red-500/50 bg-red-500/10'}>
          <CardContent className="pt-4 space-y-3">
            <div className="flex items-center gap-2">
              <AlertTriangle className="h-5 w-5 text-yellow-500" />
              <span className="font-semibold">
                {workerSupervisor?.stale
                  ? `Worker supervisor stale (${formatDuration(workerSupervisor.ageSec)})`
                  : `${workerSupervisorIssues.length} worker issue${workerSupervisorIssues.length > 1 ? 's' : ''}`}
              </span>
            </div>
            {workerSupervisorIssues.length > 0 ? (
              <div className="flex flex-wrap gap-2">
                {workerSupervisorIssues.slice(0, 8).map((item) => (
              ...
              <p className="text-sm text-muted-foreground">No supervisor updates received yet.</p>
            )}
          </CardContent>
        </Card>
      )}
      ...
      <div className="grid gap-4 lg:grid-cols-[2fr_1fr]">
        <Card>
          <CardContent className="pt-4 space-y-4">
            ...
            <div className="mt-2 pt-2 border-t flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
              <span className="font-medium text-foreground">Supervisor</span>
              <span>Down: {workerSupervisorSummary.down}</span>
              <span>Warn: {workerSupervisorSummary.warning}</span>
              <span>Ok: {workerSupervisorSummary.ok}</span>
              {workerSupervisor?.stale && (
                <span className="text-yellow-600">Stale: {formatDuration(workerSupervisor.ageSec)}</span>
              )}
            </div>
