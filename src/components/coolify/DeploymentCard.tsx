      ...
      <CardContent className="space-y-2">
        {deployment.commit && (
          <div className="flex items-center gap-2 text-sm text-muted-foreground min-w-0">
            ...
            <span className="font-mono shrink-0">{deployment.commit.slice(0, 8)}</span>
            {deployment.commit_message && (
              <span className="truncate min-w-0">{deployment.commit_message}</span>
            )}
          </div>
        )}
        <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-xs sm:text-sm text-muted-foreground">
          <span className="flex items-center gap-1">
            ...
            <span className="truncate">{formatTime(deployment.created_at)}</span>
          </span>
          {duration && (
            <span className="text-xs bg-muted px-2 py-0.5 rounded shrink-0">{duration}</span>
          )}
        </div>
      </CardContent>
    </Card>
