...
if (!UPTIME_KUMA_URL) {
  console.warn('UPTIME_KUMA_URL environment variable is not set');
}
...
export async function getUptimeStatus(): Promise<UptimeStatus> {
  try {
    ...
    const metricsRes = await fetch(`${UPTIME_KUMA_URL}/metrics`, {
      cache: 'no-store',
    });
    ...
    // Try status page API (optional status page slug)
    const statusPageSlug = process.env.UPTIME_KUMA_STATUS_PAGE;
    if (!statusPageSlug) {
      return {
        monitors: [],
    ...
    const statusRes = await fetch(`${UPTIME_KUMA_URL}/api/status-page/${statusPageSlug}`, {
      cache: 'no-store',
    });
